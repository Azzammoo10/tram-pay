...
import { ShieldCheck, ShieldAlert, Clock, Info, User, CheckCircle2, XCircle, Mail, Calendar, AlertTriangle } from 'lucide-react'
      ...
      <div className="w-full max-w-[450px] z-10 space-y-6">
        ...
        <div className="bg-white border border-neutral-border rounded-[3px] shadow-premium overflow-hidden transition-all">
          ...
          <div className="bg-brand-dark p-6 border-b border-white/5 flex items-center justify-between">
            ...
            <div className="text-[10px] tracking-widest uppercase font-bold text-[#AC6899]">Contrôle</div>
          </div>
            ...
            {status === 'loading' && (
              ...
              >
                ...
                <div className="space-y-1.5">
                  <p className="text-[15px] font-bold text-brand-dark">Lecture cryptographique...</p>
                  <p className="text-[12px] text-neutral-muted">Vérification de la signature et liaison base de données</p>
                </div>
              </motion.div>
            )}
            ...
            {status === 'valid' && (
              ...
              >
                ...
                <div className="bg-[#10B981]/10 border-b border-[#10B981]/20 p-6 flex flex-col items-center text-center gap-2">
                  <div className="w-12 h-12 bg-[#10B981]/10 border border-[#10B981]/25 rounded-full flex items-center justify-center text-[#10B981] animate-bounce">
                    <CheckCircle2 size={28} />
                  </div>
                  <h2 className="text-[18px] font-bold text-[#10B981] uppercase tracking-wide">Voyage en Règle</h2>
                  <p className="text-[12px] text-[#0f2e1a] font-medium opacity-80">Titre de transport validé avec succès</p>
                </div>
                ...
                <div className="p-6 space-y-6">
                  {traveler && (
                    <div className="p-4 bg-surface-section border border-neutral-border rounded-[3px] flex items-center gap-4">
                      {/* Avatar picture uploader layout */}
                      <div className="w-14 h-14 rounded-[3px] border border-neutral-border bg-white overflow-hidden flex-shrink-0 flex items-center justify-center shadow-card relative">
                        {traveler.avatar ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-[9px] uppercase tracking-wider font-bold text-[#AC6899] block">Titulaire de la carte</span>
                        <h3 className="text-[15px] font-bold text-brand-dark truncate mt-0.5 uppercase tracking-wide leading-tight">{traveler.name}</h3>
                        <p className="text-[11.5px] text-neutral-muted truncate flex items-center gap-1 mt-1 font-mono">
                  ...
                  {traveler && (
                    <div className="flex flex-col gap-2.5">
                      {traveler.emailVerified ? (
                        <div className="flex items-center gap-2 px-3 py-2 bg-[#10B981]/5 border border-[#10B981]/15 text-[#10B981] rounded-[3px] text-[11px] font-bold">
                          <ShieldCheck size={14} />
                          COMPTE ADHÉRENT TRAM-PAY VÉRIFIÉ (✓)
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 px-3 py-2 bg-[#F5AB32]/5 border border-[#F5AB32]/15 text-[#F5AB32] rounded-[3px] text-[11px] font-bold animate-pulse">
                          <ShieldAlert size={14} />
                  ...
                  {/* Ticket details */}
                  {ticket && (
                    <div className="border-t border-neutral-border/50 pt-5 space-y-3">
                      <span className="text-[9px] uppercase tracking-wider font-bold text-neutral-muted block">Détails de la validation</span>
                      
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 bg-surface-section border border-neutral-border rounded-[3px]">
                          <span className="text-[10px] text-neutral-muted block">Tarif Voyage</span>
                          <span className="text-[16px] font-bold text-brand-dark font-mono">7.00 DH</span>
                        </div>
                        <div className="p-3 bg-surface-section border border-neutral-border rounded-[3px]">
                          <span className="text-[10px] text-neutral-muted block">ID Unique</span>
                          <span className="text-[12px] font-mono font-bold text-cta block truncate" title={ticket.id}>
            ...
            {status === 'invalid' && (
              ...
              >
                ...
                <div className="bg-[#CF2E2E]/10 border-b border-[#CF2E2E]/20 p-6 flex flex-col items-center text-center gap-2">
                  <div className="w-12 h-12 bg-[#CF2E2E]/10 border border-[#CF2E2E]/25 rounded-full flex items-center justify-center text-[#CF2E2E]">
                    <XCircle size={28} />
                  </div>
                  <h2 className="text-[18px] font-bold text-[#CF2E2E] uppercase tracking-wide">Accès Refusé</h2>
                  <p className="text-[12px] text-[#3a0f0f] font-medium opacity-80">{getReasonMessage(reason)}</p>
                </div>

                <div className="p-6 space-y-6">
                  {/* If user profile exists, render it so inspector has metadata details */}
                  {traveler && (
                    <div className="p-4 bg-surface-section border border-neutral-border rounded-[3px] flex items-center gap-4 opacity-75">
                      <div className="w-14 h-14 rounded-[3px] border border-neutral-border bg-white overflow-hidden flex-shrink-0 flex items-center justify-center relative">
                        {traveler.avatar ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={traveler.avatar} alt="Profile photo" className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <span className="text-[9px] uppercase tracking-wider font-bold text-status-error block">Titulaire (Rejeté)</span>
                        <h3 className="text-[15px] font-bold text-brand-dark truncate mt-0.5 uppercase tracking-wide">{traveler.name}</h3>
                        <p className="text-[11.5px] text-neutral-muted truncate font-mono mt-0.5">{traveler.email}</p>
                      </div>
                    </div>
                  )}

                  <div className="p-4 bg-status-error/5 border border-status-error/15 rounded-[3px] flex gap-3 text-[12px] leading-relaxed">
                    <Info size={16} className="text-status-error flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-bold text-status-error">Consignes pour le contrôleur :</p>
